...
#endif
{
	...
	slurm_rwlock_rdlock(&as_kingbase_cluster_list_lock);
	itr2 = list_iterator_create(as_kingbase_cluster_list);
	...
	if (query) {
		...
		for (int i = 0; i < cnt; i++) {
			list_iterator_reset(itr);
			while ((coord = list_next(itr))) {
				if (!xstrcmp(coord->name, KCIResultGetColumnValue(result,i,0)))
					break;
			}
			      ...
			      List user_list)
{
	...
	while ((object = list_next(itr))) {
		...
		if (object->coord_accts)
			debug("Adding coordinators with users is not supported, ignored, use as_kingbase_add_coord() separately instead.");
			      ...
			      List acct_list, slurmdb_user_cond_t *user_cond)
{
	...
	if (query) {
		xstrfmtcat(query,
			   " as new on duplicate key update mod_time=%ld, "
			   "deleted=0, `user`=new.user;%s",
			   (long)now, txn_query);
		DB_DEBUG(DB_ASSOC, kingbase_conn->conn, "query\n%s", query);
		//info("[query] line %d, %s: query: %s", __LINE__, __func__, query);
		fetch_flag = set_fetch_flag(false, false, false);
